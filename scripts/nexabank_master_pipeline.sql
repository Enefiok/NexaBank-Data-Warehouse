-- ==========================================
-- NEXABANK MASTER AUTOMATION SCRIPT
-- ==========================================
...
4. IMPORTANT: Update the file paths in the 'STEP 5: LOAD DATA' 
...
-- STEP 1: ARCHIVE OLD DATA (Save previous analysis)
-- Note: On the VERY FIRST RUN, these INSERT statements will show an error 
-- because nexabank_dw doesn't exist yet. That is normal! Just ignore it.
CREATE DATABASE IF NOT EXISTS nexabank_history;

CREATE TABLE IF NOT EXISTS nexabank_history.dim_customers_archive LIKE nexabank_dw.dim_customers;
ALTER TABLE nexabank_history.dim_customers_archive ADD COLUMN archived_at DATETIME DEFAULT CURRENT_TIMESTAMP;

CREATE TABLE IF NOT EXISTS nexabank_history.fct_transactions_archive LIKE nexabank_dw.fct_transactions;
ALTER TABLE nexabank_history.fct_transactions_archive ADD COLUMN archived_at DATETIME DEFAULT CURRENT_TIMESTAMP;

CREATE TABLE IF NOT EXISTS nexabank_history.fct_loans_archive LIKE nexabank_dw.fct_loans;
ALTER TABLE nexabank_history.fct_loans_archive ADD COLUMN archived_at DATETIME DEFAULT CURRENT_TIMESTAMP;

INSERT INTO nexabank_history.dim_customers_archive SELECT *, NOW() FROM nexabank_dw.dim_customers;
INSERT INTO nexabank_history.fct_transactions_archive SELECT *, NOW() FROM nexabank_dw.fct_transactions;
INSERT INTO nexabank_history.fct_loans_archive SELECT *, NOW() FROM nexabank_dw.fct_loans;
...
CREATE TABLE dim_customers (customer_id INT PRIMARY KEY, account_number VARCHAR(50) UNIQUE NOT NULL, first_name VARCHAR(100) NOT NULL, last_name VARCHAR(100) NOT NULL, full_name VARCHAR(210) NOT NULL, email VARCHAR(150), phone_clean VARCHAR(50), country VARCHAR(100), city VARCHAR(150), account_type VARCHAR(100), kyc_status VARCHAR(50), annual_income_usd DECIMAL(18,2), account_open_date DATE);
CREATE TABLE fct_transactions (transaction_id INT PRIMARY KEY, customer_id INT NOT NULL, branch_id INT NOT NULL, currency_code CHAR(3) NOT NULL, transaction_type VARCHAR(50) NOT NULL, channel VARCHAR(100) NOT NULL, transaction_date DATE NOT NULL, transaction_time TIME, amount_local DECIMAL(18,2) NOT NULL, amount_usd DECIMAL(18,2) NOT NULL, description VARCHAR(255), reference_code VARCHAR(100), balance_after_local DECIMAL(18,2), balance_after_usd DECIMAL(18,2), FOREIGN KEY (customer_id) REFERENCES dim_customers(customer_id), FOREIGN KEY (branch_id) REFERENCES dim_branches(branch_id), FOREIGN KEY (currency_code) REFERENCES dim_fx_rates(currency_code));
CREATE TABLE fct_loans (loan_id INT PRIMARY KEY, customer_id INT NOT NULL, branch_id INT NOT NULL, currency_code CHAR(3) NOT NULL, loan_purpose VARCHAR(150), loan_status VARCHAR(50), loan_amount_local DECIMAL(18,2), loan_amount_usd DECIMAL(18,2), interest_rate_pct DECIMAL(10,2), tenure_months INT, amount_repaid_local DECIMAL(18,2), outstanding_balance DECIMAL(18,2), disbursement_date DATE, maturity_date DATE, FOREIGN KEY (customer_id) REFERENCES dim_customers(customer_id), FOREIGN KEY (branch_id) REFERENCES dim_branches(branch_id), FOREIGN KEY (currency_code) REFERENCES dim_fx_rates(currency_code));
...
-- STEP 5: LOAD DATA FROM INPUT FOLDER
