-- ==========================================
-- NEXABANK MASTER AUTOMATION SCRIPT (FINAL BULLETPROOF VERSION)
-- ==========================================
...
-- STEP 4: CREATE STAGING TABLES (Accepts messy raw data as text)
...
-- STEP 5: CREATE PRODUCTION TABLES (FIXED: Increased DECIMAL precision to 20,4 to stop truncation warnings)
...
CREATE TABLE dim_customers (customer_id INT PRIMARY KEY, account_number VARCHAR(50) UNIQUE NOT NULL, first_name VARCHAR(100) NOT NULL, last_name VARCHAR(100) NOT NULL, full_name VARCHAR(210) NOT NULL, email VARCHAR(150), phone_clean VARCHAR(50), country VARCHAR(100), city VARCHAR(150), account_type VARCHAR(100), kyc_status VARCHAR(50), annual_income_usd DECIMAL(20,4), account_open_date DATE);
CREATE TABLE fct_transactions (transaction_id INT PRIMARY KEY, customer_id INT NOT NULL, branch_id INT NOT NULL, currency_code CHAR(3) NOT NULL, transaction_type VARCHAR(50) NOT NULL, channel VARCHAR(100) NOT NULL, transaction_date DATE NOT NULL, transaction_time TIME, amount_local DECIMAL(18,2) NOT NULL, amount_usd DECIMAL(20,4) NOT NULL, description VARCHAR(255), reference_code VARCHAR(100), balance_after_local DECIMAL(18,2), balance_after_usd DECIMAL(20,4), FOREIGN KEY (customer_id) REFERENCES dim_customers(customer_id), FOREIGN KEY (branch_id) REFERENCES dim_branches(branch_id), FOREIGN KEY (currency_code) REFERENCES dim_fx_rates(currency_code));
CREATE TABLE fct_loans (loan_id INT PRIMARY KEY, customer_id INT NOT NULL, branch_id INT NOT NULL, currency_code CHAR(3) NOT NULL, loan_purpose VARCHAR(150), loan_status VARCHAR(50), loan_amount_local DECIMAL(18,2), loan_amount_usd DECIMAL(20,4), interest_rate_pct DECIMAL(10,2), tenure_months INT, amount_repaid_local DECIMAL(18,2), outstanding_balance DECIMAL(20,4), disbursement_date DATE, maturity_date DATE, FOREIGN KEY (customer_id) REFERENCES dim_customers(customer_id), FOREIGN KEY (branch_id) REFERENCES dim_branches(branch_id), FOREIGN KEY (currency_code) REFERENCES dim_fx_rates(currency_code));
...
-- STEP 7: ETL PIPELINE (Clean and transform)
...
SELECT CAST(t.transaction_id AS UNSIGNED), CAST(t.customer_id AS UNSIGNED), CAST(t.branch_id AS UNSIGNED), UPPER(TRIM(t.local_currency)), CONCAT(UPPER(LEFT(LOWER(TRIM(t.raw_transaction_type)), 1)), SUBSTRING(LOWER(TRIM(t.raw_transaction_type)), 2)), CASE UPPER(TRIM(t.raw_channel)) WHEN '' THEN 'Unknown' WHEN 'A.T.M' THEN 'ATM' WHEN 'ATM' THEN 'ATM' WHEN 'POS' THEN 'POS Terminal' WHEN 'POS TERMINAL' THEN 'POS Terminal' WHEN '*966#' THEN 'USSD' ELSE CONCAT(UPPER(LEFT(LOWER(TRIM(t.raw_channel)), 1)), SUBSTRING(LOWER(TRIM(t.raw_channel)), 2)) END, COALESCE(CASE WHEN TRIM(t.raw_transaction_date) REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' THEN STR_TO_DATE(TRIM(t.raw_transaction_date), '%Y-%m-%d') WHEN TRIM(t.raw_transaction_date) REGEXP '^[0-9]{2}-[A-Za-z]{3}-[0-9]{4}$' THEN STR_TO_DATE(TRIM(t.raw_transaction_date), '%d-%b-%Y') WHEN TRIM(t.raw_transaction_date) REGEXP '^[0-9]{2}/[0-9]{2}/[0-9]{4}$' THEN CASE WHEN CAST(SUBSTRING_INDEX(TRIM(t.raw_transaction_date), '/', 1) AS UNSIGNED) > 12 THEN STR_TO_DATE(TRIM(t.raw_transaction_date), '%d/%m/%Y') ELSE STR_TO_DATE(TRIM(t.raw_transaction_date), '%m/%d/%Y') END ELSE NULL END, '1970-01-01'), STR_TO_DATE(TRIM(t.raw_transaction_time), '%H:%i:%s'), CAST(REPLACE(TRIM(t.amount_local), ',', '') AS DECIMAL(18,2)), CAST(REPLACE(TRIM(t.amount_local), ',', '') AS DECIMAL(20,4)) * fx.rate_to_usd, TRIM(t.raw_description), TRIM(t.reference_code), CAST(REPLACE(TRIM(t.balance_after_local), ',', '') AS DECIMAL(18,2)), CAST(REPLACE(TRIM(t.balance_after_local), ',', '') AS DECIMAL(20,4)) * fx.rate_to_usd FROM stg_transactions t JOIN dim_fx_rates fx ON UPPER(TRIM(t.local_currency)) = fx.currency_code;
